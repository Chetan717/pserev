...
const COL_COMPANY      = process.env.COMPANY_COLLECTION      
const COL_COUPON       = process.env.COUPON_COLLECTION       
const COL_SUBSCRIPTION = process.env.SUBSCRIPTION_COLLECTION 
const COL_PAYMENTLOG   = process.env.PAYMENTLOG_COLLECTION
const COL_PENDING      = process.env.COL_PENDING; // internal, never exposed to client
...
// ── OTP rate limiter: 3 requests per IP per 5 minutes ────────────────────────
...
// ── POST /send-otp — Generate OTP server-side, send via 2Factor, store hash ──
//
// Client sends: { userId, mobile }
// Server:
//   1. Generates cryptographically secure 4-digit OTP (never sent to client)
//   2. Stores SHA-256 hash + 5-min expiry in Firestore user document
//   3. Sends OTP via 2Factor SMS API
//   4. Returns { success: true } — the OTP itself is never returned
//
app.post("/send-otp", requireApiKey, otpLimiter, requireDb, async (req, res) => {
  ...
  if (!TWOFACTOR_API_KEY) {
    console.error("TWOFACTOR_API_KEY env var is not set");
    return res.status(500).json({ success: false, error: "SMS service not configured" });
  }

  try {
    // Generate secure OTP — server-side only, never returned to client
    const otp = String(crypto.randomInt(1000, 9999));

    // Hash OTP for safe storage in Firestore
    const otpHash   = crypto.createHash("sha256").update(otp).digest("hex");
    const otpExpiry = Date.now() + 5 * 60 * 1000; // 5 minutes

    // Store hash in Firestore (not the OTP itself)
    await db.collection(COL_USERS).doc(userId).update({ otpHash, otpExpiry });

    // Send SMS via 2Factor API
    const smsUrl = `https://2factor.in/API/V1/${TWOFACTOR_API_KEY}/SMS/+91${mobile}/${otp}/OTP`;
    const smsRes = await fetch(smsUrl);
    if (!smsRes.ok) {
      const body = await smsRes.text().catch(() => "");
      console.error("2Factor SMS failed:", smsRes.status, body);
...
// ── POST /verify-otp — Verify OTP hash (timing-safe), clear after success ────
//
// Client sends: { userId, otp }
// Server:
//   1. Loads stored otpHash + otpExpiry from Firestore
//   2. Checks expiry
//   3. Compares hashes in constant time (prevents timing attacks)
//   4. On success: clears stored hash to prevent replay
//   5. Returns { success: true/false }
//
app.post("/verify-otp", requireApiKey, requireDb, async (req, res) => {
  ...
  if (!otp || !/^[0-9]{4}$/.test(otp)) {
    return res.status(400).json({ success: false, error: "4-digit OTP required" });
  }
  ...
  try {
    ...
    const { otpHash, otpExpiry } = userSnap.data();
    ...
    // Clear OTP after successful verification (prevents replay attacks)
    await db.collection(COL_USERS).doc(userId).update({ otpHash: "", otpExpiry: 0 });
